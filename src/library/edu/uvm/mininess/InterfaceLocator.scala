...
/**
 * Abstracts the algorithm for locating and parsing Mininess interface information. Classes that have this trait use
 * some internal mechanism for accessing nesC libraries. Multiple implementations are possible depending on how the
 * libraries are stored. This trait is intended to expose only a Mininess interface. If the underlying libraries are
...
trait InterfaceLocator {
   ...
   * 
   * @return The definition of the specified interface in Mininess form. The import and export lists depend on the
   *         interface's direction.
   * 
   * @throws InterfaceNotFoundException if this locator can't find the interface.
   * @throws InterfaceNotRepresentableException if the interface isn't expressible as Mininess.
   */
  def locate(
...
object InterfaceLocator {
  ...
  /**
   * Exception thrown when an interface can't be represented in Mininess terms. This exception is also thrown when an
   * interface does not parse successfully. The presumption is that such interfaces contain full nesC constructs that
   * are not legal Mininess.
   */
