...
class InterfaceSourceLocator(sourceFolders: List[String]) extends InterfaceLocator {
  ...
  /**
   * Search the source folders looking for the file containing the source code of the given
   * interface.
   * 
  ...
  private def locateSourceFile(interfaceName: String) = {
    ...
    // It might be better, in the case where the interface is found in more than one source
    // file, to return the first one found for some reasonable definition of "first." However,
    // having two different definitions of the same interface is likely to be an error so for
    // now let's treat it as such.
    // 
  ...
  /**
   * Locate the source code of the given interface and parse it to obtain its definition. This
   * method assumes that interface I is stored in a source file I.nc in one of the previously
   * provided source folders. This method currently reparses the interface each time it is
   * called. There is no attempt to cache previously parsed interfaces.
   */
  def locate(
    ...
    def getCommandName(declaratorList: ASTNode) = {
      // Be aware! Match failures here cause JVM stack overflow exceptions! (Scala 2.9.1) The
      // problem appears to be in the printing of the ASTNodes. Could there be a problem with
      // the AST (maybe a cycle?)
    ...
    // Extract the command information from the interface. I'm going to assume that all the
    // declarations in the interface are command/event declarations with no objects or typedefs.
    // I'm also going to assume that the first "declaration specifier" in each declaration is
    // either "command" or "event."
