...
class ScalanessPostParser(val global: Global, settings: ConfigurationSettings) extends PluginComponent with Transform {
  ...
  /**
   * Process the body of a class or module definition to see if it contains a nesT inclusion. If it does, then
    ...
    if (body.size == 0) impl
      ...
      checkForNesTInclusion(lastItem) match {
        ...
        case Some( (shortName, fullName) ) => {
          ...
          val nesTComponentName = shortName.substring(0, shortName.indexOf('.'))
