//-----------------------------------------------------------------------
// FILE    : PluginProperties.scala
// SUBJECT : An object for managing the properties of the plugin
...
/**
 * A utility to load properties of this plugin via the property file "plugin.properties"
 */
object PluginProperties {
  ...
  private lazy val props: Option[Properties] = {
    // Running from JAR file: the properties file should be in the jar as well.
    var stream = this.getClass.getResourceAsStream("/"+ propFilename)
    if (stream == null) {
       // Running from .class files: expect class files to be in directory [...]/build/build.main, and [...] to contain
       // the properties file.
      try {
