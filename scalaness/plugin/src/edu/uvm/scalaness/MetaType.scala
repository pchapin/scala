 ...
 * represent nesT types that are constructed (or "selected") during the execution of the
 * first stage program. In that sense MetaType instances are holders for a range of nesT
 * types that are bounded by Tau.
 *
 * Notice that the type argument is the Scala side representation of a nesT type but the
 * value argument is the AST of a nesT type.
 *
...
class MetaType[+Tau <: LiftableTypes.Liftable](val wrappedType: NesTTypes.Representation)
   ...
   * nesT type is copied to the new instance.
   *
