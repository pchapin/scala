  ...
  def extractDeclaredNames(declaration: ASTNode): (List[(String, NesTTypes.Representation)],
    ...
    val declaratorList =
      for (declarationChild <- decList) yield {
        ...
        val existsModifier = (existsChild(declarationChild, NesTLexer.DECLARATOR_ARRAY_MODIFIER) ||
        ...
        // If there is an array modifier, return an array type
          ...
          val arrayModifier = findChild(declarationChild, NesTLexer.DECLARATOR_ARRAY_MODIFIER)
        ...
        // If there is a function modifier, build the parameter list and return function type
          ...
          val parameterListModifier = findChild(declarationChild, NesTLexer.DECLARATOR_PARAMETER_LIST_MODIFIER)              
          val parameterListChild = findChild(parameterListModifier, NesTLexer.PARAMETER_LIST)
