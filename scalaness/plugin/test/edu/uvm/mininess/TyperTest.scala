...
// SUBJECT : Class for JUnit testing of Mininess type checking.
...
class TyperTest extends TestHarness {
  ...
  private val MininessTyping = new File(testTyping, "Mininess")
  ...
  /**
   * Parse a Mininess file.
   * 
   * @param testCaseName The name of the file containing the Mininess component to parse.
   * @return An ASTNode for the root of the resulting abstract syntax tree.
   */
  private def parseNesCFile(testCaseName: String) = {
    val testCase = new File(MininessTyping, testCaseName)
    ...
    try {
      ...
      val abstractSyntax = 
        TreeConverter.ANTLRToScala(t = parser.mininess_file().getTree.asInstanceOf[CommonTree])
  ...
  private def defaultSymbols = {
    ...
    new Symbols(
      ...
      typeNames     = Map[String, MininessTypes.Representation](("int" -> Int16)),
      variableNames = Map(
        ("x" -> Int16),
        ("y" -> Int16),
        ("z" -> Int16),
  ...
  private def typeCheckCase(
    ...
    val typeChecker = new MininessTyper(defaultTypeVars, Map(), defaultTypeCompatibility, debugFlag)
    val result = typeChecker.checkMininessInclusion(decoratedNode)

    // The overall type of the Mininess inclusion is actually a module type.
    // assertTrue(result == Some(MininessTypes.Okay))
  }
