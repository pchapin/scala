...
\Sprocket\ also uses radio links to implement \newterm{dynamic wires} (as described in
...
\item A session key is computed using elliptic curve Diffie-Hellman key agreement and added to
  the session key storage under the proper $(N, C, I)$ value. The key is stored as a remote
  client key.
