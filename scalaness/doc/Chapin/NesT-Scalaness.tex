...
the imports of $\texttt{e}_2$.  This is specified in the \TirName{ModWire}
...
% The Scalaness Type judgements are of the form $\Gamma \vdash \texttt{e} :
% \texttt{T}$, where $\Gamma$ is a free variable typing environment and
...
%If $\varnothing \vdash \texttt{e} : \texttt{N}$ and $\texttt{e} \rightarrow^*
%\texttt{e}'$ with $\texttt{e}'$ irreducible\footnote{This is defined as ``normal
%form'' in \cite{XXX}.}, then $\texttt{e}'$ is either a
%reference $\jref{\texttt{p}}{N'}$ with $\texttt{N'} <: \texttt{N}$, or an
%expression containing $(\texttt{N}')\texttt{\jref{\texttt{p}}{N''}}$ where
%$\texttt{N''}\not<:\texttt{N}'$.
%\end{theorem}
