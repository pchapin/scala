...
names Scalaness and nesT refer to the languages as implemented, while \newterm{DScalaness} and
\newterm{DnesT} refer to the distilled languages studied theoretically. The implementation is
...
\autoref{figure-example} shows the complete example. The definition begins with a
...
Next, on line~\ref{l:radiot} defines a type $\tt{radioT}$, which is the type of DnesT modules
...
Now on line~\ref{l:commt} another type is defined $\tt{commT}$ which is the type of modules
...
\note{should this be adopted instead of adapted?}adapted here unchanged in Scalaness
...
Refer to \cite{FJ,AFJ} for details on the FGJ and AFJ object oriented calculi,
