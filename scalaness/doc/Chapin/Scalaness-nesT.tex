...
% \note{Talk about nesT type rules and type checking? I need to reference Michael's thesis in
% here somewhere.}
...
% TODO: I need to describe the nesT subset somewhat exhaustively and carefully.
...
classes are liftable and have representations as nesT structure types. \note{Figure out how to
  properly model Scala's inheritance system in this context.}
...
that obey the syntax in \autoref{figure-module-type-syntax}.
...
existential-binders ::= existential-binder (',' existential-binder)*
existential-binder  ::= ID '<:' type
type-parameters  ::= type-parameter (',' type-parameter)*
type-parameter   ::= ID '<:' type
value-parameters ::= value_parameter (',' value-parameter)*
value-parameter  ::= ID  ':' type
\end{Verbatim}
}
\end{fpfig}
\primaryspacing
...
the actual type derived by the Scalaness type rules in \autoref{section-XXX}. Type errors are
...
(\filename{SendC.nc} above), parses it as nesT and does nesT type checking on that file using
...
In particular, the generated field !configuration! holds a reference to an object representing a
