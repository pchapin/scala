\newcommand{\syntaxfig}
{
\begin{fpfig*}[t]{Program Syntax of DnesT}{figure-syntax}
\figsize
$$
\begin{array}{lc@{\ }l@{\hspace{-5mm}}r}
\s, \t & \defassign & \TVAR\ \mid \top \mid \intone\ \mid \inttwo  \mid \intt \mid \undefv \mid & \textit{types} \\
&  & \lc\tbindvec{\fdname}{\t}\rc \mid \t \idx{} \mid
\t \kwstar  \mid \blok \t \\ %\mid \texist{\tpdecl}{\tau} \mid \margs{\tpdecl; \vpdecl}\lc  \imports; \exportsty \rc  & \\
e & \defassign &  v \mid \lvalue \mid 
  e \op e   \mid \castto{\t}{e} \mid \fname\blok{} \mid \lvalue = e \mid %e \pluseq e \mid 
  \& \lvalue \mid & \textit{expressions} \\
 & &   \kwif\ (e)\  e\ \kwelse\ e \mid \kwwhile\ (e)\  e \mid e;
e \mid \kwpost\ \fname\blok{} &\\
\lvalue &\defassign& x \mid e\idx{e} \mid e.\fdname 
 \mid \kwstar e & \textit{l-values} \\
v & \defassign &    \neight \mid \nsixtn \mid \undefv & \textit{base values} \\
% \mv &\defassign& v \mid  
%  \arr{\vect \bn} \mid \lc \vect{\fdname = \bn} \rc & 
%\textit{values in memory}\\ 
\mathit{op} & \defassign & + \mid \kwstar \mid \texttt{\&\&} \mid \texttt{==} \ldots & \textit{operations} \\
\id &\defassign& \fname \mid x & \textit{identifiers}\\
d &\defassign& \xlet{\t}{x}{e} \mid \xlet{\t}{x}{\arr{\vect{e}}} \mid \xlet{\t}{x}{\lc \defvec{\fdname}{e} \rc} \mid \fname : \t =  \blok{e} & \textit{declarations} \\[3mm]
\tpdecl & \defassign & \subtvec{\TVAR}{\t} \qquad \vpdecl  \defassign  \tbindvec{\VAR}{\t} & \textit{type and value parameters} \\
\imports & \defassign & \tbindvec{\fname}{\t}\qquad  \ \ \,\exports  \defassign  
  \tbindvec{\fname}{\t} = \vect{\blok{e}}& \textit{imports and exports} \\
\exportsty & \defassign & \tbindvec{\fname}{\t} & \textit{export signatures} \\
\mu & \defassign &  \margs{\tpdecl; \vpdecl}\lc  \imports; \decls \exports \rc  & \textit{module definitions}\\
\jmodtcat & \defassign & \margs{\tpdecl; \vpdecl}\lc \imports; \exportsty \rc & \textit{module signatures}
%\localname & \defassign & \fname\ \mid \blockno \qquad \identifier \defassign \localname \mid x & \textit{export names and identifiers} 
%\fname & \in & \mathcal{F} & \textit{function name} \\
%\fdname & \in & \mathcal{L} & \textit{field name} \\
%\VAR & \in & \mathcal{V} & \textit{term variable} \\
%\TVAR & \in & \mathcal{T} & \textit{type variable} \\
%\blockno & \in & \mathcal{M} & \textit{memory location} \\
%\neight & \in & \mathbb{Z}_{2^8}& \textit{8-bit unsigned integer} \\
%\nsixtn & \in  & \mathbb{Z}_{2^{16}}& \textit{16-bit unsigned integer} \\
\end{array} 
$$
\end{fpfig*}
}

\newcommand{\modulesyntaxfig}
{
\begin{fpfig}[t]{Syntax of DnesT Types and Modules}{figure-modulesyntax}
\figsize
$$
\begin{array}{lclr}
\s, \t & \defassign & \TVAR\ \mid \top \mid \intone\ \mid \inttwo  \mid \intt \mid& \textit{types} \\
&  &  \undefv \mid \lc\tbindvec{\fdname}{\t}\rc \mid \t \idx{} \mid
\t \kwstar \\[2mm] %\mid \texist{\tpdecl}{\tau} \mid \margs{\tpdecl; \vpdecl}\lc  \imports; \exportsty \rc  & \\
...
\exportsty & \defassign & \vect{s} & \textit{export types} \\
%\multicolumn{3}{l}{\exports ::= \vect{c} \quad \imports ::= \vect{s} \quad \exportsty ::= \vect{s}} & \textit{exports, imports, export types}\\ 

d &\defassign& \xlet{\t}{x}{e} \mid \xlet{\t}{x}{\arr{\vect{e}}} \mid  & \textit{declarations} \\
 & & \xlet{\t}{x}{\lc \defvec{\fdname}{e} \rc} \mid c  \\
\mu & \defassign &  \margs{\tpdecl; \vpdecl}\lc \imports; \decls \exports \rc  & \textit{module definitions}\\
\jmodtcat & \defassign & \margs{\tpdecl; \vpdecl}\lc \imports; \exportsty \rc & \textit{module signatures}
%\localname & \defassign & \fname\ \mid \blockno \qquad \identifier \defassign \localname \mid x & \textit{export names and identifiers} 
%\fname & \in & \mathcal{F} & \textit{function name} \\
...
\mv &\defassign& v \mid \bn \mid   
\arr{\vect{\kappa}} \mid \lc \defvec{\fdname}{\kappa} \rc & 
 \textit{dynamic values}\\ 
\flash & \defassign & \tdefvec{\fname}{\t}{\blok{e}} \qquad
 \blockmem \defassign \tdefvec{\blockno}{\t}{\mv} & \textit{codebase 
and memory} \\
%\rv & \defassign & \lb \mem,v\rb & \textit{runtime values} \\ 
%e & \defassign & \cdots \mid \rv & \textit{runtime expressions} \\ 
\envletter & \defassign & \context{}{} \mid \envletter; e \mid \kappa \idx{E} \mid E\idx{e} \mid E.\fdname 
 \mid   \kappa = \envletter \mid \envletter = e \mid 
 \castto{\t}{\envletter} \mid 
%\bn \pluseq \envletter \mid 
& \textit{evaluation\ contexts}\\
& & \kwstar \envletter \mid \&\envletter \mid \envletter \ \emph{op} \ e\ \mid 
 \kappa\ \emph{op}\ \envletter \mid \kwif\ (\envletter)\ e\ \kwelse\ e\ \mid
 \kwwhile\ (E)\  e  \\ 
%{D} & \defassign & 
%    \xlet{\t}{x}{E} \mid 
...
{
\begin{fpfig}[h]{Subtyping Rules}{figure-subjudge}
\figsize
\begin{mathpar}
\inferrule[ReflS]
{}
...
\inferrule[TransS]
{\subjudge{\tpdecl}{\t_1}{\t_2} \\ \subjudge{\tpdecl}{\t_2}{\t_3}}
{\subjudge{\tpdecl}{\t_1}{\t_3}}
...
\begin{fpfig}[h]{Selected Declaration and Module Typing Rules}{figure-declmodtyping}
\begin{mathpar}
...
\begin{array}{rclr}
\\[-4mm]%
\tt{L} &::=& \gclass{C}{\gbounds{X}{N}}{N}{\fieldvec{T}{f};\ K\ \ttvec{M}} & \gdesc{classes} \\[1mm] % & \gdesc{class definitions}\\[1mm]
...
\tt{e} &::=& \tt{x} \mid \select{e}{f} \mid \send{e}{m}{\ttvec{e}} \mid \gnew{C}{\ttvec{T}}{\ttvec{e}}  \mid \cast{N}{e} \mid & \gdesc{expressions} \\ 
 & & \tt{l} \mid \mutate{\select{e}{f}}{e} \mid  \jdef{x}{T}{e}{e} \mid\\
 & &  \abbrvt{X(\ttvec{X})}{T}{e} \mid\\
 & & % \jtlet{x}{\tt{T}}{e}{e} \mid \\ & & 
\jmodval \mid \jwire{\tt{e}}{\tt{e}} \mid \jinst{e}{\ttvec{e};\ttvec{e}} \mid \jimage{\tt{e}} \\[1mm]
\tt{T} &::=& \tt{X} \mid \tt{N} \mid \jmodt{\tpdecl}{\jmodtcat}  & \gdesc{types} \\[1mm]
...
{\jwire{\margs{} \lc  \imports_1; \vect{d}_1; \exports_1 \rc}
       {\margs{} \lc  \imports_2; \vect{d}_2; \exports_2 \rc}
 \rightarrow
   \margs{}
...
\inferrule[ModImage]
{\texttt{main} \text{\ defined\ in\ } \exports}
...
\begin{mathpar}
\inferrule[ModT]
{\jmodval : \jmodtcat \text{\ in\ DnesT\ type\ checking}}
{\Gamma \vdash \jmodval : \jmodt{\varnothing}{\jmodtcat}}

\inferrule[ModImageT]
{\Gamma \vdash \tt{e} : \jmodt{\tpdecl}{\margs{}\lc \imports; \exportsty \rc} \\ \text{main}() : \t \in \exportsty}
...
{\Gamma \vdash \tt{e} : \jmodt{\varnothing}{\margs{\vect{t} \subtype \vect{\t}_1; 
 \vect{x} : \vect{\t}_2} \lc 
  \imports; \exportsty \rc}\\
 \Gamma \vdash \ttvec{t} : \jinst{MetaType}{\ttvec{T}_1}\\
 \Gamma \vdash \ttvec{e} : \ttvec{T}_2 \\
 \vdash \codt{\ttvec{T}_1} \subtype \vect{\t}_1\\
 \vdash \codt{\ttvec{T}_2} \subtype \vect{\t}_2
}
{\Gamma \vdash \jinst{e}{\ttvec{t}; \ttvec{e}} : \jmodt{\ttvec{t} \subtype \codt{\ttvec{T}_1}}{\margs{} \lc 
  \imports[\ttvec{t}/\vect{t}]; \exportsty[\ttvec{t}/\vect{t}] \rc} }
...
{\Gamma \vdash \jwire{\tt{e}_1}{\tt{e}_2} : 
 \jmodt{\tpdecl_1 \maploosemerge \tpdecl_2}{\margs{} 
    \lc  \imports; \exportsty_1 \rc}}
\end{mathpar}
\end{fpfig}
}

\newcommand{\scalanessfig}
{
\begin{fpfig*}[t]
...
\includegraphics[scale=.54]{scalaness}
\end{center}
\end{fpfig*}
}
...
{
\begin{fpfig}[h]
{A Sensor Node (L,C) and Hub Device (R).}
{figure-snowcloud}
\hspace{1mm}
\includegraphics[scale=.28]{brainbox} 
\includegraphics[scale=.28]{tower} 
\includegraphics[scale=.28]{harvester} 
\end{fpfig}
}

\newcommand{\spartanrpcfig}
{
\begin{fpfig}[t]
{Staging Authorization and Authorized Access in a Multi-Domain WSN.}
{figure-spartanrpc}
\begin{center}
\includegraphics[scale=.28]{spartanrpc} 
\vspace{-2mm}
\end{center}
\end{fpfig}
}

\newcommand{\evaluationfig}
{
\begin{fpfig}[t]
{RAM and ROM usage for Harvester and Sensor Node Code Versions, all numbers in bytes.}
{figure-evaluation}
\begin{center}
{\small
\begin{tabular}{rr}
